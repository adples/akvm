...
global $acf_block_counts;
if ( ! isset( $acf_block_counts ) ) {
	$acf_block_counts = array();
}

$count_name = $block['name'];

if ( ! isset( $acf_block_counts[ $count_name ] ) ) {
	$acf_block_counts[ $count_name ] = 0;
}

++$acf_block_counts[ $count_name ];
...
if ( ! empty( $padding_top ) ) {
	$padding_top = 'pt-' . ( (int) $padding_top / 2 - 10 ) . ' xl:pt-' . (int) $padding_top / 2;
}

$padding_bot = ( empty( $block['style']['spacing']['padding']['bottom'] ) ) ? '' : preg_replace( '~\D~', '', $block['style']['spacing']['padding']['bottom'] );
if ( ! empty( $padding_bot ) ) {
	$padding_bot = 'pb-' . ( (int) $padding_bot / 2 - 10 ) . ' xl:pb-' . (int) $padding_bot / 2;
}
...
} elseif ( ( is_single() && has_post_thumbnail() ) && 1 === $acf_block_counts[ $count_name ] ) {
	$bg_md      = get_the_post_thumbnail_url( get_the_ID(), 'medium_large' );
	$bg_full    = get_the_post_thumbnail( get_the_ID(), 'full' );
	$attributes = 'class="' . $classes . ' ' . $bg_pos . ' bg-local bg-cover bg-no-repeat bg-resize" style="background-image: url(' . $bg_md . ')" data-img-md="' . $bg_md . '" data-img-full="' . $bg_full . '"';
} else {
